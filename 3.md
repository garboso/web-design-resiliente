...
Design acrescenta clareza. Utilizando cores, tipografia, hierarquia, contraste e todas as outras ferramentas à sua disposição, designers podem um amontoado de informações desordenadas e transformá-las em algo que é fácil de utilizar e aprazível de se contemplar. Assim como a própria vida, o design pode ter um pequeno triunfo contra a entropia do universo, criando espaços ordenados a partir dos materiais em estado bruto do caos.
...
Quando a web começou a conquistar o mundo nos anos 1990, os designers começaram a migrar do papel para os pixels. _Criando Sites Arrasadores_ do David Siegel surgiu justamente nessa época. Suas gambiarras espertas utilizando tabelas e GIFs permitiu que os designers replicassem o mesmo tipo de layout que eles criaram anntes para a página impressa.

Esses layouts utilizando tabela depois se tornaram layouts CSS, mas o raciocínio fundamental manteve-se o mesmo: a janela do navegador–assim como antes foi a página–é tratada como uma restrição conhecida nas quais os designers determinam a ordem.
...
## Se ainda não está consertado, não quebre
...
A proporção da janela de um navegador é apenas um exemplo de _known unknown_ da web. A forma mais simples de lidar com essa situação é utilizar unidades flexíveis no layout: percentuais ao invés de pixels. Ao invés disso, designers escolheram fingir que as dimensões dos navegadores eram um _known known_. Eles criaram layouts com largura fixa para um tipo específico de tamanho de janela.

No início da web, muitos monitores possuiam 640 pixels de largura. Web designers criaram layouts que tinham 640 pixels de largura. Conforme mais e mais pessoas começaram a utilizar monitores com 800 pixels de largura, mais e mais designers começaram a criar layouts com 800 pixels de largura. Alguns anos depois, isso se tornou 1024 pixels. Em algum ponto web designers definiram o número mágico de 960 pixels como uma largura perfeita.
...
Tendo consciência disso, não é surpreendente que o web design começou com as tentativas de recriar os layouts que os designers estavam acostumados do mundo do impresso. Conforme John diz:
...
"_O controle que os designers veem na mídia impressa, e muitas vezes desejam na mídia web, é simplesmente a função da limitação da página impressa. Nós deveríamos aceitar o fato que a web não possui as mesmas restrições e projetar contando com essa flexibilidade._"
...
Há também outro motivo para os web designers apegarem-se ao conforto de seus layouts fixos. As ferramentas do ofício encorajavam uma abordagem tal qual da mídia impressa no momento de criar designs para a web.
...
Quando a área do web design estava emergindo, não existiam softwares criados especificamente para visualizar layouts da wen. Ao invés disso, designers cooptaram ferramentas existentes.
...
Uma minoria de web designers ainda fazia votos por layouts fluídos. Eu estou contando comigo mesmo entre esse número. Nós éramos tolerados da mesma forma que um profeta do juízo final em uma esquina vestindo uma placa-sanduíche em que se lê "O Fim Se Aproxima"–uma distração inconveniente porém inofensiva.

Lá também existiam designers sugerindo que o Photoshop não deveria ser a melhor ferramenta para a web e que nós deveríamos cogitar em desenhar diretamente no navegador utilizando CSS e HTML. Essa abordagem era criticada por ser muito restritiva. Como nós já vimos, o Photoshop possuía suas próprias limitações mas elas foram interiorizadas pelos designers tão confortáveis em utilizar a ferramenta a ponto que eles não reconheciam suas deficiências.
...
"Um iPod. Um telefone. E um comunicador de Internet. Um iPod. Um telefone... vocês estão sacando? Esses não são aparelhos diferentes. É um aparelho só. E nós vamos chamar ele de: iPhone."
...
- Como nós saberemos se as pessoas estão utilizando as telas largas do Desktop ou telas mais estreitas dos portáteis?
- Como nós saberemos se as pessoas estão navegando com uma conexão rápida de banda larga em casa ou em uma rede móvel lenta?
- Como nós saberemos se um dispositivo oferece suporte a alguma tecnologia específica ou plugin?

O crescimento dos dispositivos móveis estava confrontando web designers com a verdadeira natureza da web como mídia flexível recheada de mistérios (unknowns).
...
Mas isso não significa que os dispositivos de tela pequena devem receber as páginas que foram desenhadas para dimensões maiores:

"Porém, isso não significa que a mesma informação estará disponível na mesma representação exata por todos os dispositivos."

Se web designers desejaram estar do lado do espírito da One Web, eles precisavam oferecer a mesma essência do conteúdo na mesma URL para todos, independente de seu dispositivo. Ao mesmo tempo, eles precisavam estar aptos a criar layouts diferentes dependendo do verdadeiro estado da tela disponível.

A ilusão distribuída da abordagem de "tamanho único serve pra todos" no web design começou a evaporar. Ela foi gradualmente substituida pela aceitação para a natureza fluída e em constante mudança da web.
...
Um mês depois, ele expandiu essa ideia em um artigo chamado _Responsive Web Design_. O artigo foi publicado no A List Apart, o mesmo site que havia publicado _A Dao Of Web Design_, de John Allsopp, dez anos atrás. O artigo de Ethan compartilhada do mesmo espírito de John. Na verdade, Ethan começa seu artigo citando _A Dao Of Web Design_.

Ambos artigos convocavam os web designer a adotar a ideia de uma só web. Mas, enquanto _A Dao Of Web Design_ foi amplamente rejeitado pelos designers confortáveis em suas ferramentas WYSIWYG, _Responsive Web Design_ deparou-se com uma audiência de profissionais desesperados para resolver o enigma dos dispositivos móveis.
...
É por isso que o micro-ondas não poderia ter sido inventado na França medieval; era requerido que ocorresse outros passos antecessores–manufatura, energia, teoria–para acontecer esse tipo de salto. Facebook não poderia existir sem a World Wide Web, a qual não poderia existir sem a Internet, a qual não poderia existir sem os computadores e assim por diante. Cada passo depende das camadas acumuladas com o tempo.

No momento, Ethan cunhou o termo Web Design Responsivo para uma série de avanços tecnológico que tinham aparecido. Assim como eu escrvi no prefácio do livro subsequente de Ethan sobre o assunto:
...
1. Grids fluídos. A opção de usar porcentagens ao invés de pixels já estava presente conosco desde a época dos layouts com tabelas.
...
3. Media queries. Graças a forma que o CSS trata os erros, navegadores foram adicionando funcionalidades em cima de funcionalidades ao decorrer do tempo. Uma dessas funcionalidades era as CSS Media Queries–a habilidade de definir estilos de acordo com parâmetros específicos, como as dimensões da janela do navegador.

As camadas estão postas. Um desejo por mudança–influenciado pelo crescimento implacável do ecossistema móvel–também estava posto. O que era necessário era um lema para unir tudo isso. Foi isso que Ethan nos deu com Web Design Responsivo.

## Mudando a mentalidade

Os primeiros expeirmentos de design responsivo envolviam aperfeiçoar websites que eram voltados apenas para desktop: converter pixels para porcentagens e adicionar media queries para remover o layout grid em telas menores. Mas essa abordagem reativa não dispunha de uma fundação sólida para se basear. Felizmente, outro lema foi capaz de encapsular por uma abordagem mais resistente.

Luke Wroblewski cunhou o termo "Mobile First" em reação a ascenção dos dispositivos móveis:
...
“_Adote a fluidez da web. Os layouts e sistemas que podem superar qualquer ecossistema que eles se encontrarem. Mas, a única forma de que fazer que eles possam fazer isso é dizer que existiram correntes em nossos pescoços. Elas não permitiam o nosso melhor desempenho. Comece projetando a partir do conteúdo, ao invés de ir diretamente para a tela._”
